...
function setupCaptureTest(
  ...
  config?: Parameters<typeof LanguageModel.prototype.updateConfig>[0]
): {
  model: LanguageModel
  mock: LanguageModelV3
  callArgs: () => LanguageModelV3CallOptions
  collect: (messages: Message[], options?: Parameters<LanguageModel['stream']>[1]) => ReturnType<typeof collectIterator>
} {
  const mock = createMockModel(parts)
  const model = new LanguageModel(mock, config)
  return {
    model,
    mock,
...
describe('LanguageModel', () => {
  ...
  describe('constructor and config', () => {
    it('uses model.modelId as default and allows override', () => {
      ...
      expect(new LanguageModel(mock).getConfig().modelId).toBe('test-model')
      expect(new LanguageModel(mock, { modelId: 'custom-id' }).getConfig().modelId).toBe('custom-id')
    })

    it('passes through all config fields', () => {
      const mock = createMockModel([])
      const model = new LanguageModel(mock, {
        maxTokens: 100,
        temperature: 0.5,
        topP: 0.9,
        topK: 40,
      ...
      it.each([
        {
          ...
          block: new ImageBlock({
            format: 'png',
            source: { s3Location: { uri: 's3://bucket/key', bucketOwner: '' } },
          }),
        ...
        },
        {
          ...
          block: new VideoBlock({
            format: 'mp4',
            source: { s3Location: { uri: 's3://bucket/video', bucketOwner: '' } },
          }),
