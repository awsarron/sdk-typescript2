/**
 * Vercel AI SDK LanguageModelV3 model provider implementation.
 *
 * This module provides integration with any Vercel AI SDK v3 compatible model provider,
 ...
 * These patterns are common across AI SDK providers (Bedrock, OpenAI, Anthropic, etc.).
 */
...
/**
 * Configuration for the LanguageModel adapter.
 *
 ...
 */
export interface LanguageModelConfig extends BaseModelConfig, LanguageModelCallSettings {}

/**
 * Adapter that wraps a LanguageModelV3 instance
 * for use as a Strands model provider.
 *
 * Implements the Model interface for any Vercel AI SDK v3 compatible provider.
 * Supports streaming responses, tool use, and reasoning content.
 *
 * @example
 * ```typescript
 * import { Agent } from '@strands-agents/sdk'
 * import { LanguageModel } from '@strands-agents/sdk/language-model'
 * import { bedrock } from '@ai-sdk/amazon-bedrock'
 *
 * const agent = new Agent({
 *   model: new LanguageModel(bedrock('us.anthropic.claude-sonnet-4-20250514-v1:0')),
 * })
 *
 ...
 * ```
 */
export class LanguageModel extends Model<LanguageModelConfig> {
  private _model: LanguageModelV3
  private _config: LanguageModelConfig

  /**
   * Creates a new LanguageModel instance.
   *
   * @param model - A LanguageModelV3 instance from any AI SDK provider
   * @param config - Optional configuration overrides
   */
  constructor(model: LanguageModelV3, config?: Partial<LanguageModelConfig>) {
    super()
    this._model = model
  ...
  getConfig(): LanguageModelConfig {
    return { ...this._config }
  }

  updateConfig(config: LanguageModelConfig): void {
 ...
 *
 * @param error - The error thrown by the AI SDK provider
    ...
    switch (block.type) {
      ...
      case 'imageBlock': {
        ...
        if (source.type === 'imageSourceBytes') {
          content.push({ type: 'file', data: source.bytes, mediaType: `image/${block.format}` })
        } else if (source.type === 'imageSourceUrl') {
          content.push({ type: 'file', data: new URL(source.url), mediaType: `image/${block.format}` })
        } else {
      ...
      case 'documentBlock': {
        const source = block.source
        const mediaType = DOCUMENT_MEDIA_TYPES[block.format] ?? `application/${block.format}`
      ...
      case 'videoBlock': {
        ...
        if (source.type === 'videoSourceBytes') {
          content.push({ type: 'file', data: source.bytes, mediaType: `video/${block.format}` })
        } else {
